...
const Sidebar = () => {
    ...
    const blankBoard = {
        id: Date.now().toString(), // Generate a unique ID for the board
        name:'',
        bgcolor:'#f60000',
        list:[]
    };
    ...
    const addBoard = async () => {
        try {
            // Send the new board to the backend to save it
            const response = await axios.post("http://localhost:5000/api/boards", boardData);

            // Update the frontend state with the newly created board from the backend
            let newB = { ...allboard };
